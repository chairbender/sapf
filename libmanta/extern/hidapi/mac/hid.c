...
static int get_string_property(IOHIDDeviceRef device, CFStringRef prop, wchar_t *buf, size_t len)
{
	...
	if (str) {
		...
		CFRange range;
		range.location = 0;
		range.length = (str_len > (CFIndex)len)? (CFIndex)len: str_len;
                         ...
                         uint8_t *report, CFIndex report_length)
{
	...
	if (++counter >= 10000) {
		printf("dt mean %12.6f  min %12.6f  max %12.6f\n", sumdt*.0001, mindt, maxdt);
		maxdt = 0.;
		mindt = 1e10;
		sumdt = 0.;
		counter = 0;
	...
	//printf("---\n");
	//printf("report_type %d   id %d   len %d  dt %6.1f msec\n", (int)report_type, (int)report_id, (int)report_length, dt);
